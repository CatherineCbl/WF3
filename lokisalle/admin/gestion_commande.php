...
if (!empty($_POST))
{
    ...
    $contenu .= "<div class='validation'>La commande a bien Ã©tÃ© enregistrÃ©</div>";
    $_GET['action'] = 'affichage';
}
...
//---------- AFFICHAGE DES commandeS-----------------//
    ...
    $contenu .= '<th>DÃ©tails de la commande</th>';
    ...
    while ($ligne = $resultat->fetch_assoc()) {
        ...
        $contenu .='<td><a href=?action=detail&id_commande=' . $ligne['id_commande'] . '><img src="../inc\img\icone_search.png" width="50px" height="50px"></a></td>';
        $contenu .= '<td><a href=?action=modification&id_commande=' . $ligne['id_commande'] . '"><img src="../inc/img/edit.png"></a></td>';
        $contenu .= '<td><a href=?action=suppression&id_commande=' . $ligne['id_commande'] . '" OnClick="return(confirm(\'En Ãªtes vous certain?\'))"><img src="../inc/img/delete.png"></a></td>';
        $contenu .= '</tr>';
    }
...
if (isset($_GET['action']) && ($_GET['action'] == 'ajout' ||$_GET['action'] == 'modification'))
{
    if (isset($_GET['id_commande'])) {
        $resultat = executeRequete("SELECT * FROM commande WHERE id_commande = '$_GET[id_commande]'");//on rÃ©cupÃ¨re les informations sur l'article Ã  modifier
        $commande_actuel = $resultat->fetch_assoc();//on rend les informations exploitables afin de les prÃ©saisir dans les cases du formulaire
    }
...
if($_GET)
{
    ...
    $contenu .= '<h2>Affichage des dÃ©tails de la commande</h2>';
    //$contenu .= 'Nombre de commandes : ' . $resultat->num_rows;
    $contenu .= '<table border="1" cellpading="5"><tr>';
