...
.directive('multiBg', function(_, $parse){
	return {
		scope: {
			multiBg: '=',
			interval: '=',
		...
		controller: function($scope, $element, $attrs) {
			...
			// This only appplies when we don't have the image url and we need an $http service to go get it.
			// This service must return a promise, and that promise must resolve the image url
			// debugger;
			// if(!_.isUndefined($scope.remoteImage))
			if(!_.isUndefined(_.result($scope, 'remoteImage')))
			{
				$scope.remoteImage().then(function(image_url){
					utils.setBackground(image_url);
				},function(error){
					// console.log(error);
				})
			}
